...
var prop = symbol.for;
var lazyVarsPropName$1 = prop('__lazyVars');
...
function defineGetter(context, varName, options) {
  ...
  var vars = varContext[lazyVarsPropName$1] || {};

  if (!varContext[lazyVarsPropName$1]) {
    varContext[lazyVarsPropName$1] = vars;
  }
  ...
  if (accessorName in varContext) {
    throw new Error(['Cannot create lazy variable "', varName, '" as variable with the same name exists on the provided context'].join(''));
  }
...
var rspec$2 = function createRspecTracker(config) {
  var tracker = suite_tracker(config);
